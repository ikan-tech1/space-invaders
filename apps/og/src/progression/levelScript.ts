...
export function getLevelBanner(level: number): string {
  const t = getEncounterType(level);
  if (t === "bigBoss") return `BIG BOSS — LEVEL ${level}`;
  if (t === "miniBoss") return `MINI BOSS — LEVEL ${level}`;
  return `LEVEL ${level}`;
}

export function getLevelConfig(level: number): LevelConfig {
  const formations: FormationType[] = ["classic", "staggered", "diamond", "pincer"];
